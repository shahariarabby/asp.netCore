...
//builder.Services.AddDbContext<ModelDbContext>(x => x.UseSqlServer(Configuration.GetConnectionString("ConStr")));
...
app.MapControllerRoute(name: "login",
                pattern: "login/{*article}",
                defaults: new { controller = "TestView", action = "login" });
